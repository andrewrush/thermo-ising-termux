...
Да. Смартфон на Android — это 8-ядерный ARM64-компьютер с аппаратным генератором случайных чисел. Мы используем `/dev/urandom` (генератор случайных чисел на чипе) для инициализации симуляций — телефон буквально использует свой тепловой шум для научного эксперимента.
