...
void seed_fixed(uint64_t a, uint64_t b) {
    s[0] = a; s[1] = b;
}
...
// === Температура батареи (обход SELinux) ===
...
// === Основной цикл Метрополиса ===
void metropolis(double T, int steps, int print_interval, int csv_mode) {
    int accepted = 0;
...
int main(int argc, char *argv[]) {
    ...
    metropolis(T, steps, print_interval, csv);
